...
class SourceProcess:
    def get(self):
        ...
        for i in range(len(self.url)):
            wget.download(self.url[i], f"sources/{self.source}/{self.code[i]}.{self.format}")
    ...
    def convert(self):
        if self.format == 'xml':
            ...
            for i in range(len(self.url)):
                with open(
                        f"sources/{self.source}/{self.code[i]}.{self.format}") as xml_file:
            ...
            for i in range(len(self.url)):
                with open(
                        f"sources/{self.source}/{self.code[i]}.{self.format}") as json_file:
