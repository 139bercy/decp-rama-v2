...
class ProcessPesMarches(SourceProcess):
    def __init__(self):
        self.url = ["https://files.data.gouv.fr/decp/dgfip-pes-decp.xml"]
        self.code = ["data.gouv.fr_pes"]
...
class ProcessAWS(SourceProcess):
    def __init__(self):
        self.url = [
    ...
    def fix(self):
        ...
        self.df['concessionaires'] = self.df['concessionaires'].apply(
            lambda x: [{'concessionaire': y} for y in x] if str(x) != 'nan' else x)
        # Supression des doublons
